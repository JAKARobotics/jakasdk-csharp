...
namespace jkType
{
	...
	public class JKTYPE
	{
		...
		*/
		...
		[StructLayout(LayoutKind.Sequential)]
		public struct MoveJParam
		{
			public int id;				///< motion cmd id, range limit: [0, 5000], set to -1 if you want controller to set automatically
			public bool is_block;		///< block until this cmd is done
			public JointValue joints;	///< targe joint value
			public MoveMode mode;		///< motion mode
			public double vel;			///< velocity
			public double acc;			///< acceleration, set to 90 if you have no idea
			public double tol;			///< tolerance, used for blending. set to 0 if you want to reach a fine point
		}

		[StructLayout(LayoutKind.Sequential)]
		public struct MoveLParam
		{
			public int id;					///< motion cmd id, range limit: [0, 5000], set to -1 if you want controller to set automatically
			public bool is_block;			///< block until this cmd is done
			public CartesianPose end_pos;	///< taget position
			public MoveMode move_mode;		///< motion mode
			public double vel;				///< velocity
			public double acc;				///< acceleration, set to 500 if you have no idea
			public double tol;				///< tolerance, used for blending. set to 0 if you want to reach a fine point
			public double ori_vel;			///< set to 3.14 if you have no idea
			public double ori_acc;			///< set to 12.56 if you have no idea
		}

		[StructLayout(LayoutKind.Sequential)]
		public struct MoveCParam
		{
			public int id;					///< motion cmd id, range limit: [0, 5000], set to -1 if you want controller to set automatically
			public bool is_block;			///< block until this cmd is done
			public CartesianPose mid_pos;	///< mid position
			public CartesianPose end_pos;	///< end position
			public MoveMode move_mode;		///< motion mode
			public double vel;				///< velocity
			public double acc;				///< acceleration, set to 500 if you have no idea
			public double tol;				///< tolerance, used for blending. set to 0 if you want to reach a fine point
			public double circle_cnt;		///< circule count
			public int circle_mode;			///< clock wise or counter clock wise
		}

		[StructLayout(LayoutKind.Sequential)]
		public struct MotionStatus
		{
			public int motion_line; 		///< the executing motion cmd id
			public int motion_line_sdk; 	///< reserved
			public bool inpos; 				///< previous motion cmd is done, should alway check queue info together
			public bool err_add_line; 		///< previous motion cmd is dropped by controller, like target is already reached
			public int queue; 				///< motion cmd number in buffer
			public int active_queue; 		///< motion cmd number in blending buffer
			public bool queue_full;			///< motion buffer is full and motion cmds reveived at this moment will be dropped
			public bool paused;				///< motion cmd is paused and able to resume
		}
