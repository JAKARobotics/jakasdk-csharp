...
namespace sdk_CS
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            result = jakaAPI.create_handler("192.168.164.222".ToCharArray(), ref handle);
            //机器人上电
            if (result == -1)
            {
            ...
            jakaAPI.enable_robot(ref handle);
            Console.WriteLine("ret= {0}\n", result);
            jakaAPI.get_sdk_version(ref handle, version);
            ...
            ret = jakaAPI.linear_move_extend(ref handle, ref fone, JKTYPE.MoveMode.ABS, false, 10, 200, 0, ref option_cond);
            Console.WriteLine("ret==", ret);
            //JKTYPE.RobotStatus status = new JKTYPE.RobotStatus();
            //Console.WriteLine(a = seed.Next(100, 200));

            //float ranfloat = (float)seed.NextDouble();
            //Console.WriteLine(ranfloat);
            //for (int i = 0; i < 10; i++)
            //{
            ...
            //    //Console.WriteLine("output:{0}",a = seed.Next(100, 200));
            //    int handle = 0;
            //    jakaAPI.set_analog_output(ref handle, JKTYPE.IOType.IO_MODBUS_SLAVE, 33, f);
            //    Thread.Sleep(100);
            //    //end = jakaAPI.get_analog_output(ref handle, JKTYPE.IOType.IO_MODBUS_SLAVE, 3, ref v);
            //    //Console.WriteLine("模拟输出：{0}\n",  v);

            //    jakaAPI.get_robot_status(ref handle, ref robstatus);
            //    //Console.WriteLine("rapidrate:{0}\n", robstatus.modbus_slave);
            //    Console.WriteLine("模拟输出：{0}\n", robstatus.aout[34]);

            //    Console.WriteLine("模拟输出：{0}\n", robstatus.modbus_slave.aout[34]);
            //}

            //jakaAPI.set_analog_output(ref handle, JKTYPE.IOType.IO_MODBUS_SLAVE, 3, a);
            //Thread.Seep(0.2);

            //end = jakaAPI.get_analog_output(ref handle, JKTYPE.IOType.IO_MODBUS_SLAVE, 3, ref v);
            //jakaAPI.get_robot_status(ref handle, ref robstatus);
            //Console.WriteLine("{1}rapidrate:{0}\n", robstatus.modbus_slave, ret);
            //int index = 2;
            //Console.WriteLine("模拟输出：{0},{1}\n", index, robstatus.aout[3]);
            //Console.WriteLine("模拟输出：{0},{1}\n", index, v);
