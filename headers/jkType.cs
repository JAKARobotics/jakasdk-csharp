	...
	public class JKTYPE
	{
        ...
        */
		...
		public struct CartesianTran
		{
			...
			public CartesianTran(double x = 0, double y = 0, double z = 0)
			{
				this.x = x;
				this.y = y;
				this.z = z;
			...
			public CartesianTran normalize()
			{
				double length = this.length();
				return new CartesianTran(this.x / length, this.y / length, this.z / length);
			}

			public static CartesianTran operator +(CartesianTran a, CartesianTran b)
			{
				return new CartesianTran(a.x + b.x, a.y + b.y, a.z + b.z);
			}

			public static CartesianTran operator -(CartesianTran a) 
			{
				return new CartesianTran(-a.x, -a.y, -a.z);
			}
			public static CartesianTran operator -(CartesianTran a, CartesianTran b)
			{
				return a + (-b);
			...
			public static CartesianTran operator *(CartesianTran a, double b)
			{
				return new CartesianTran(a.x * b, a.y * b, a.z * b);
			}
			public static CartesianTran operator *(CartesianTran a, int b)
			{
				return new CartesianTran(a.x * b, a.y * b, a.z * b);
			}
            ...
            public override bool Equals([NotNullWhen(true)] object? obj)
            {
                ...
                {
                    JKTYPE.CartesianTran tran = (JKTYPE.CartesianTran)obj;
                    return tran.x == this.x && tran.y == this.y && tran.z == this.z;
                }
            }
        };
		...
		*/
		...
		public struct Rpy
		{
            ...
            /// <summary>
            /// Please do not use this constructor. Use the Factory Methods 'withRadians' and 'withDegrees' instead.
            /// </summary>
            public Rpy()
			{
            ...
            /// Please do not use this constructor. Use the Factory Methods 'withRadians' and 'withDegrees' instead.
            /// </summary>
            /// <param name="rx">Rotational x in rad.</param>
			/// <param name="ry">Rotational y in rad.</param>
			/// <param name="rz">Rotational z in rad.</param>
            private Rpy(double rx = 0, double ry = 0, double rz = 0)
			{
				this.rx = rx;
				this.ry = ry;
				this.rz = rz;
            ...
            /// <summary>
            /// Factory Method to create Rpy struct with rad as parameters.
            /// </summary>
            /// <param name="rx">Rotational x in rad.</param>
            /// <param name="ry">Rotational y in rad.</param>
            /// <param name="rz">Rotational z in rad.</param>
            public static Rpy withRadians(double rx, double ry, double rz)
			{
				return new Rpy(rx, ry, rz);
            }

            /// <summary>
            /// Factory Method to create Rpy struct with degrees as parameters.
            /// </summary>
            /// <param name="rx">Rotational x in degrees.</param>
            /// <param name="ry">Rotational y in degrees.</param>
            /// <param name="rz">Rotational z in degrees.</param>
            public static Rpy withDegrees(double rx, double ry, double rz)
			{
				return new Rpy(rx * Math.PI / 180, ry * Math.PI / 180, rz * Math.PI / 180);
			}

			public static Rpy operator +(Rpy a, Rpy b)
			{
				return new Rpy(a.rx + b.rx, a.ry + b.ry, a.rz + a.rz);
			}

			public static Rpy operator -(Rpy a)
			{
				return new Rpy(-a.rx, -a.ry, -a.rz);
			}
			...
			public Rpy rotateDegrees(int axis, double degrees)
			{
				return this;
			}

			public Rpy rotateRad(int axis, double rad)
			{
				return this;
			}
            ...
            public override bool Equals([NotNullWhen(true)] object? obj)
            {
				...
				{
					JKTYPE.Rpy rpy = (JKTYPE.Rpy)obj;
					return rpy.rx == this.rx && rpy.ry == this.ry && rpy.rz == this.rz;
				}
            }
		...
		*/
		...
		public struct CartesianPose
		{
            ...
            public CartesianPose(CartesianTran tran, Rpy rpy)
			{
				this.tran = tran;
				this.rpy = rpy;
			}

			public CartesianPose(CartesianTran tran)
			{
				this.tran = tran;
				this.rpy = new Rpy();
			}

			public CartesianPose(Rpy rpy)
			{
				this.tran = new CartesianTran();
				this.rpy = rpy;
			}

			private CartesianPose(double x, double y, double z, double rx, double ry, double rz)
			{
				this.tran = new CartesianTran(x, y, z);
				this.rpy = Rpy.withRadians(rx, ry, rz);
			}

			public static CartesianPose withRadians(double x, double y, double z, double rx, double ry, double rz)
			{
				return new CartesianPose(x, y, z, rx, ry, rz);
			}

			public static CartesianPose withDegrees(double x, double y, double z, double rx, double ry, double rz)
			{
				return new CartesianPose(x, y, z, rx * Math.PI / 180, ry * Math.PI / 180, rz * Math.PI / 180);
            }

            public static CartesianPose withDegrees(double[] coords)
            {
                Array.Resize(ref coords, 6);
                double[] tran = coords.Take(3).ToArray();
                double[] rpy = coords.Skip(3).ToArray();

                return new CartesianPose(new CartesianTran(tran[0], tran[1], tran[2]), Rpy.withDegrees(rpy[0], rpy[1], rpy[2]));
            }

            public static CartesianPose withRadians(double[] coords)
            {
                Array.Resize(ref coords, 6);
                double[] tran = coords.Take(2).ToArray();
                double[] rpy = coords.Skip(2).ToArray();

                return new CartesianPose(new CartesianTran(tran[0], tran[1], tran[2]), Rpy.withRadians(rpy[0], rpy[1], rpy[2]));
            }

            public static CartesianPose operator +(CartesianPose left, CartesianTran right)
			{
				return new CartesianPose(left.tran + right, left.rpy);
			}
            ...
            public override bool Equals([NotNullWhen(true)] object? obj)
            {
				...
				{
                    JKTYPE.CartesianPose pose = (JKTYPE.CartesianPose) obj;
					return pose.tran.Equals(this.tran) && pose.rpy.Equals(this.rpy);
                }
            }

            public override string ToString()
            {
                return this.tran.x + ", " + this.tran.y + ", " + this.tran.z + ", " + this.rpy.rx + ", " + this.rpy.ry + ", " + this.rpy.rz;
            }
        };
		...
		*/
		public struct JointValue
		{
            ...
            {
                this.jVal = [j1, j2, j3, j4, j5, j6, j1];
            }

            public static JointValue withDegrees(double j1, double j2, double j3, double j4, double j5, double j6)
			{
				return new JointValue(j1 * Math.PI / 180, j2 * Math.PI / 180, j3 * Math.PI / 180, j4 * Math.PI / 180, j5 * Math.PI / 180, j6 * Math.PI / 180);
			}

			public JointValue withRadians(double j1, double j2, double j3, double j4, double j5, double j6)
			{
                return new JointValue(j1, j2, j3, j4, j5, j6);
            }
			

			public JointValue(double[] jointValues)
			{
				this.jVal = jointValues;
			}
		};
